...
> 상태: **§2 = ✅구현 완료·확정**(이슈 #10 · PR #18) · **§1·3·4·5 = 구현 완료, PR 대기**(이슈 #9) · 공통 규약은 `API명세_본챌린지.md` §0.
> 위 ⚠️초안 배너는 **#9 PR 때 제거**한다(§2만 확정돼서는 파일 전체 배너를 못 지움).
...
> `durationDays`에 숫자 아닌 값이 오면 **현재 500**(공통 `GlobalExceptionHandler`에 타입 바인딩 실패 핸들러 없음 — 나연 확인 대기, 400 매핑되면 이 줄과 컨트롤러 테스트 갱신).
