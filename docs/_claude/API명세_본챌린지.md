...
| 401 | 토큰 없음/유효하지 않음 |
| 409 | 이미 진행 중(IN_PROGRESS) 챌린지 존재 — ⚠️가정: **동시 1개**. 만료됐는데 미확정(§4 lazy 확정 전)인 챌린지는 생성 검사 전에 확정돼 409를 내지 않는다(#8에서 정비 — 포기·히스토리와 동일 원리). 휴식 중이면 409가 아니라 **열린 휴식을 자동 종료하고 생성 진행**(휴식 §1 배타 규칙, #8) |
