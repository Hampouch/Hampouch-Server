...
## 🚦 구현 현황 (안드 인계용 — 2026-07-18)

> ✅ 서버 구현 완료 = 지금 붙일 수 있음(유저 식별은 X-User-Id 헤더 스텁, 로그인 연동 전). ⏳ 미구현 = 명세만 있고 코드 없음(붙이면 404).

- **✅ 구현 완료 (9)**: §1 `POST /challenges` · §2 `GET /current`(휴식기 응답 포함, #8) · §3 `GET /{id}/calendar` · §4 `GET /{id}/result` · §5 `POST /{id}/days`(시드 전용) · 지난 챌린지 리스트 `GET /challenges/history`(#4 — 상세 `API명세_히스토리.md`) · 중도 포기 `POST /{id}/give-up`(#3 — 상세 `API명세_중도포기.md`) · 휴식 시작 `POST /rests`·복귀 `POST /rests/resume`(#8 — 상세 `API명세_휴식.md`)
- **⏳ 미구현**: 미니(4개)·한도조정·추천·집중카테고리 — 명세는 각 기능 구현 PR에서 추가(붙이면 404)
...
| `CHALLENGE_ALREADY_IN_PROGRESS` | 409 | 진행 중인데 새 챌린지 생성 · 진행 중인데 휴식 시작(휴식 §1 — #8에서 발생 지점 추가) |
...
| 401 | 토큰 없음/유효하지 않음 |
| 409 | 이미 진행 중(IN_PROGRESS) 챌린지 존재 — ⚠️가정: **동시 1개**. 만료됐는데 미확정(§4 lazy 확정 전)인 챌린지는 이 중복 검사보다 먼저 확정돼 409를 내지 않는다(#8에서 정비 — 포기·히스토리와 동일 원리). 휴식 중이면 409가 아니라 **열린 휴식을 자동 종료하고 생성 진행**(휴식 §1 배타 규칙, #8) |
...
> 위젯도 이 API 재사용 예정(팀 협의) · 홈 날짜 스트립의 과거(as-of) 조회는 `date` 파라미터 확장 예정(0711 PM — 설계 초안, 구현 이슈로) · **휴식 중이면 404가 아니라 200 + `challenge: null` + `rest` + `keptRecords`**(#8 구현 완료 — 상세 `API명세_휴식.md` §3, 진행 중 챌린지가 있으면 그쪽 우선).
...
|---|---|---|
| 401 | (나연 common) | 인증 실패 |
| 404 | `NO_ACTIVE_CHALLENGE` | 진행 중 챌린지도 **열린 휴식도** 없음(#8 — 휴식 중이면 404가 아니라 200 휴식기 응답) |
