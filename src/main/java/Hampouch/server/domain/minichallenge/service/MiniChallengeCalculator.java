 ...
 * 그날은 세지 않되 끊지도 않고 전날부터 거꾸로 센다. 근거 — 미니 챌린지 API 명세 §1(그날 조회)에
 * 실린 응답 예시 JSON이 오늘 4개 중 3개만 체크된 상태(checkedCount 3, totalCount 4 = 당일 미완료)인데도
 * streakDays를 0이 아닌 3으로 보여줌 = 진행 중인 하루가 스트릭을 0으로
 * 만들지 않는다는 뜻(#1 홈 집계의 "오늘은 기록 보냈을 때만 포함" 규칙과 같은 사상).
 * 조회일 이전 날짜는 미충족이면 그대로 끊는다.
 *
 * 과거 날짜 조회에도 스킵 규칙을 똑같이 적용한다(조회일이 실제 오늘인지 여기선 구분하지 않는다).
 * 스킵의 근거는 "바뀔 수 있어서"가 아니다 — 늦은 체크는 이전 날짜들도 가능하므로 그건 근거가 못 되고,
