 ...
 * - actualResumeDate: 실제 복귀일 — "복귀"라는 별개 사건(팝업 응답 또는 새 챌린지 생성)이 실행된 결과의 기록. null = 그 사건이 아직 없음(활성 휴식).
 *   휴식을 끝내는 건 이 값뿐이다. "내일부터(TOMORROW)"도 의향이 아니라 실행 완료다 — 발효만 내일로 미룬 것이라 서버가 그 날짜로 종료를
 *   자동 집행하며, 오늘까지는 휴식 중으로 남는다. 그래서 "활성이다"의 판정은 null 검사로 끝나지 않고 날짜 비교(isActiveOn)까지 필요하다.
 */
...
public class UserRest {
     ...
     * null = 아직 안 정해짐(활성 휴식) · 오늘(NOW) = 오늘부터 휴식 아님 · 내일(TOMORROW) = 내일부터 휴식 아님(오늘까진 휴식).
     * 이름은 "실제"지만 TOMORROW가 발효일을 하루 앞서 적으므로 미래가 될 수 있다 — 그 경우는 내일뿐이고, 적은 당일 하루만 미래로 남는다.
     * 불변값이 아니라 "발효 전엔 엎어질 수 있는 기록"이다: 휴식이 아직 활성인 동안(내일 예약의 당일)은 마지막 결정이
     * 이긴다(NOW로 당기기·EXTEND로 취소·챌린지 생성의 자동 종료). 발효(기준일 도달) 후엔 별도 잠금 없이 저절로 불변 —
     * 이 행을 고치는 모든 경로가 활성 휴식만 찾으므로(findActiveOn) 비활성 행은 아무도 못 만진다.
     ...
     * 그래서 활성 판정은 null 검사만으론 안 되고 날짜 비교(isActiveOn: null 또는 기준일보다 뒤)까지 필요하다 —
     ...
     * 아직 휴식 중(isActiveOn=true)인 동안은 마음을 바꿀 수 있다(서비스가 활성 휴식만 찾아 호출).
     */
     ...
     * 이 필드를 쓰는 곳은 resume(오늘 또는 내일)뿐이고 서비스는 활성 휴식(null 또는 미래)만 찾아 넘기므로,
    ...
    /**
     * 기준일에 이 휴식이 활성인가 — 복귀 기록이 없거나(actual null), 복귀일이 기준일보다 뒤(내일 복귀 예약)면 참.
