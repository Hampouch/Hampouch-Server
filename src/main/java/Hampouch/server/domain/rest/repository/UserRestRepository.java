...
public interface UserRestRepository extends JpaRepository<UserRest, Long> {
    ...
    /**
     * 기준일에 열려 있는 휴식 1건 — UserRest.isActiveOn과 같은 규칙의 쿼리판
     * (복귀 기록이 없거나, 복귀일이 기준일보다 뒤인 "내일 복귀 예약" 상태까지 열림으로 취급).
     *
     ...
     *
     * "0 또는 1건" 계약은 두 다리로 선다. ① 데이터: 열린 휴식이 있으면 시작이 409로 막혀(UserRestService)
     * 유저당 열린 휴식은 최대 1건. ② 호출: 기준일에는 오늘만 넘긴다(파라미터 이름이 today인 이유) —
