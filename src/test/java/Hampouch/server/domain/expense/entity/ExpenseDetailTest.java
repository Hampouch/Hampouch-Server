...
class ExpenseDetailTest {
    ...
    @Test
    @DisplayName("of()로 생성하면 memo만 채워지고 imageKey/imageUrl은 null이다")
    void of_startsWithMemoOnly() {
        ...
        assertThat(detail.getImageUrl()).isNull();
    }
    ...
    @Test
    @DisplayName("updateMemo()는 memo만 바꾸고 imageKey/imageUrl은 건드리지 않는다")
    void updateMemo_onlyChangesMemo() {
        ...
        assertThat(detail.getImageUrl()).isEqualTo("https://bucket.s3.region.amazonaws.com/expenses/abc.jpg");
    }

    @Test
    @DisplayName("attachImage()는 imageKey/imageUrl만 바꾸고 memo는 건드리지 않는다 — updateMemo()와 대칭 케이스")
    void attachImage_onlyChangesImageFields() {
        ExpenseDetail detail = ExpenseDetail.of(expense(), "메모는 유지되어야 함");

        detail.attachImage("expenses/new.jpg", "https://bucket.s3.region.amazonaws.com/expenses/new.jpg");

        assertThat(detail.getImageKey()).isEqualTo("expenses/new.jpg");
        assertThat(detail.getImageUrl()).isEqualTo("https://bucket.s3.region.amazonaws.com/expenses/new.jpg");
        assertThat(detail.getMemo()).isEqualTo("메모는 유지되어야 함");
    }

    @Test
    @DisplayName("attachImage()를 다시 호출하면(재첨부) 이전 imageKey/imageUrl을 새 값으로 덮어쓴다")
    void attachImage_overwritesPreviousImage() {
        ExpenseDetail detail = ExpenseDetail.of(expense(), null);
        detail.attachImage("expenses/old.jpg", "https://bucket.s3.region.amazonaws.com/expenses/old.jpg");
    ...
    @Test
    @DisplayName("removeImage()는 imageKey/imageUrl만 비우고 memo는 그대로 유지한다 — DELETE .../photos의 계약")
    void removeImage_clearsImageFieldsButKeepsMemo() {
